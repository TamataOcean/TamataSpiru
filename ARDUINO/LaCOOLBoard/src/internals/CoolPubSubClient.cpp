...
boolean CoolPubSubClient::readByte(uint8_t * result, uint16_t * index){
...
uint16_t CoolPubSubClient::readPacket(uint8_t* lengthLength) {
    uint16_t len = 0;
...
boolean CoolPubSubClient::publish(const char* topic, const char* payload) {
    return publish(topic,(const uint8_t*)payload,strlen(payload),false);
}

boolean CoolPubSubClient::publish(const char* topic, const char* payload, boolean retained) {
    return publish(topic,(const uint8_t*)payload,strlen(payload),retained);
}

boolean CoolPubSubClient::publish(const char* topic, const uint8_t* payload, unsigned int plength) {
...
boolean CoolPubSubClient::publish_P(const char* topic, const uint8_t* payload, unsigned int plength, boolean retained) {
    uint8_t llen = 0;
    uint8_t digit;
    unsigned int rc = 0;
    uint16_t tlen;
...
boolean CoolPubSubClient::write(uint8_t header, uint8_t* buf, uint16_t length) {
    uint8_t lenBuf[4];
    uint8_t llen = 0;
    uint8_t digit;
    uint8_t pos = 0;
...
boolean CoolPubSubClient::subscribe(const char* topic) {
...
CoolPubSubClient& CoolPubSubClient::setServer(uint8_t * ip, uint16_t port) {
    IPAddress addr(ip[0],ip[1],ip[2],ip[3]);
    return setServer(addr,port);
}

CoolPubSubClient& CoolPubSubClient::setServer(IPAddress ip, uint16_t port) {
    this->ip = ip;
    this->port = port;
    this->domain = NULL;
    return *this;
...
CoolPubSubClient& CoolPubSubClient::setServer(const char * domain, uint16_t port) {
    this->domain = domain;
    this->port = port;
    return *this;
}

CoolPubSubClient& CoolPubSubClient::setCallback(MQTT_CALLBACK_SIGNATURE) {
    this->callback = callback;
    return *this;
}

CoolPubSubClient& CoolPubSubClient::setClient(Client& client){
    this->_client = &client;
    return *this;
}

CoolPubSubClient& CoolPubSubClient::setStream(Stream& stream){
    this->stream = &stream;
    return *this;
}
